...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.beta > 0 and r < args.cutmix_prob:
            ...
            print('bbox ', bbx1, bby1, bbx2, bby2)

            input[:, :, bbx1:bbx2, bby1:bby2] = input[rand_index, :, bbx1:bbx2, bby1:bby2]

            img, lam2, x1, y1, x2, y2, x11, y11, x22, y22 = Cutshift(input, lam2, bbx1, bbx2, bby1, bbx2, 5)
            ...
            #loss = criterion(output, target_a_var) * lam + criterion(output, target_b_var) * (1. - lam)\
            #     + criterion(output, target_a_var) * lam2 + criterion(output, target_b_var) * (1. - lam2)

            im = np.zeros((32, 32, 3), np.uint8)
            im = cv2.rectangle(im, (bby1, bbx1), (bby2, bbx2), (255,0,0), 1) # 파랑 -> 바운딩박스
            im = cv2.rectangle(im, (y1, x1), (y2, x2), (0, 255,0), 1) # 초록 -> 원래 좌표
            im= cv2.rectangle(im, (y11, x11), (y22, x22), (0,0,255), 1) # 빨강 -> 옮겨진 좌표
...
def Cutshift(img, lam2, bx1, by1, bx2, by2, length):
    ...
    # Iou 계산 
    patch_lam = 0
    a = min(x2, bx2) - max(bx1, x1)
    b = min(y2, by2) - max(by1, y1)

    if a > 0 and b > 0 :
        print(a, b)
        
        #input.size()[-1] * input.size()[-2]
        patch_lam =  1 - ((a *  b) / (x_length2 * y_length2))
        
    return img, patch_lam, x1, y1, x2, y2, x11, y11, x22, y22
