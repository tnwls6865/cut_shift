...
class ResNet(nn.Module):
    ...
    def channel_mix(self, out, rand_index, ratio):
        ...
        mixout[:, :channel, :, :] = realout[:, :channel, :, :]      

        out = torch.cat((realout, mixout), dim=0)
        return out
        

    def forward(self, x, is_train, rand_index=0, ratio=0):
        if is_train == True:
            f = self.conv1(x)
            out = F.relu(self.bn1(self.conv1(x)))
            out = self.channel_mix(out, rand_index, ratio) #  b, 64, 32, 32
            
            #'rand_index' and 'ratio
            out = self.layer1(out) # b, 64, 32, 32
            out = self.channel_mix(out, rand_index, ratio)
