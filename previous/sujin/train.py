...
def main():
    ...
    if args.featuremix:
        test_id = test_id + '_featuremix_front' 
    ...
    lr_step_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=60, gamma=0.001) # Decay LR by a factor of 0.1 every step_size
    ...
    csv_logger = CSVLogger(args=args, fieldnames=['epoch', 'train_loss', 'train_acc', 'test_acc'], filename=filename)
    ...
    for epoch in range(args.epochs):
        ...
        writer.add_scalar('featuremix/test_loss', test_loss, epoch)
        writer.add_scalar('featuremix/test_acc', test_acc, epoch)
...
def train(progress_bar, model, criterion, optimizer, epoch):
    ...
    for i, (images, labels) in enumerate(progress_bar):
        ...
        if args.beta > 0 and r < args.mix_prob:
            ...
            # compute output
            input_var = torch.autograd.Variable(real_image, requires_grad=True)
            ...
            if args.cutmix == True:
                ...
                loss = criterion(output[:b_size], label_b_var) + criterion(output[b_size:], label_a_var) *lam + criterion(output[b_size:], label_b_var) * (1. - lam)
        ...
        grid = make_grid(input_var, nrow=5, padding=5, normalize=True)
        writer.add_image('tnwls/images', grid, 0)
        ...
        writer.add_scalar('featuremix/train_loss', xentropy_loss_avg/(i+1), epoch)
        writer.add_scalar('featuremix/train_acc', accuracy, epoch)
