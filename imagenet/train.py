...
def main():
    global args, best_err1, best_err5, hvd
    ...
    if args.dataset.startswith('cifar'):
        ...
        traindir = os.path.join('../ImageNet/ILSVRC2012_img_train/')
        valdir = os.path.join('../ImageNet/ILSVRC2012_img_val/')
    ...
    filename = 'logs/imagenet_resnet18_2' + '.csv'
    csv_logger = CSVLogger(args=args, fieldnames=['epoch', 'train_loss', 'acc1', 'acc5', 'val_loss'], filename=filename)
    ...
    for epoch in range(0, args.epochs):
        ...
        # train for one epoch
        train_loss = train(train_loader, val_loader,  model, criterion, optimizer, epoch)
        ...
        row = {'epoch': str(epoch), 'train_loss': str(train_loss), 'acc1': str(acc1), 'acc5': str(acc5), 'val_loss': str(val_loss)}
...
def train(train_loader, val_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0 and args.verbose == True and hvd.rank()==0:
            print('Epoch: [{0}/{1}][{2}/{3}]\t'
                  'LR: {LR:.6f}\t'
        ...
        #if i % 500 == 0 and i != 0:   
        #    err1, err5, val_loss = validate(val_loader, model, criterion, epoch)
        #    print('Current test accuracy (top-1 and 5 error):  \n', err1, err5)
